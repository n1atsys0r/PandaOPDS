...
- **校验**：zip 可打开 + 条目数 > 0 + 抽查首页；不符置 `failed`（保留文件备查，可删除重试）。档位升级（`start(force=True)` 且跨档）：升级前快照旧 ready meta 进 `meta.json` 的 `upgrade_backup` 并删除残留 `.part`（防跨档续传污染），旧 `archive.zip` 只在新包校验通过后才被原子替换（7z 经临时文件转换）；升级失败（含重启中断，启动时 `recover_interrupted_upgrades`）自动回滚到原 ready 档（不断流，`error` 留痕）。
...
- **周期同步**：`FavoritesSyncer`（`app/favorites/sync.py`）按 `FAVORITES_SYNC_INTERVAL_SECONDS`（**默认 3600=每小时**；设 0 关闭周期循环）周期增量扫描 `/favorites.php`（`inline_set=fs_f dm_e` 强制收藏时间排序 + extended 布局，**不走列表缓存**），连续 `FAVORITES_SYNC_MATCH_THRESHOLD`（默认 5）个已知 gid 停止翻页（对齐参考实现 `MATCH_THRESHOLD=5`；区别于参考的增量模式无页数限制，此处增加 `FAVORITES_SYNC_MAX_PAGES`=50 硬上限防跑飞），扫描恒全量（`FAVORITES_SYNC_CATEGORIES` 不过滤扫描）。`FAVORITES_SYNC_CATEGORIES` 仅为自动归档作用域：逗号分隔的 favcat ID；留空全归档；`-ID` 为排除项，如 `-0` = 除 0 分区外全归档；scope =（白名单或全部）− 排除项，名单外条目仍记入 `known`（WebUI 照常显示已收藏）只是永不自动归档。快照持久化于 `FAVORITES_SYNC_STATE`（缺省容器内 `/config/favorites_sync.json`、本地 `./favorites_sync.json`，原子写）：`known`/`archived`/`errors`/`baseline`/`last_run`。**分页游标**：收藏时间排序下 `#unext` 输出**复合游标** `next={gid}-{收藏时间戳}`（如 `2753175-1786365950`），PandaOPDS 将其视作**不透明字符串**原样透传（对齐 JHenTai 参考实现），绝不 `int()` 强转或截断（否则分页漂移或 500——2026-08-20 线上故障根因）。**写操作后一次性触发**：`POST /api/favorites` 任一成功即调度一次防抖后台扫描（3s 窗口合并批量、与周期/手动任务互斥，周期关闭时仍生效），索引/自动归档即时跟上；无 IPB 登录态时扫描直接跳过返回（不产生 sadpanda 噪音）。
- **基线（baseline）语义**：**首次运行只建档不归档**——全量扫描（受页数上限约束）把当前所有收藏记入 `known` 并置 `baseline=true`，但**绝不自动归档**（空快照上开启自动归档会把存量全部当"新增"一次性消耗 GP）；**自第二次运行起**才对真正新增项按 `FAVORITES_SYNC_ARCHIVE` 执行归档。删除状态文件即重置基线（下次运行重新建档、仍不归档）。
- **自动归档**：`FAVORITES_SYNC_ARCHIVE`（默认 0）**必须手动开启**才对新发现条目调用 `ArchiveManager.start()`（GP 消耗风险显式知情）；归档前先过 `FAVORITES_SYNC_CATEGORIES` 作用域（名单外跳过，只建档不归档）；去重 + 档位感知升级：新发现条目若已有 ready 本地归档，仅 `res→org` 且 `ARCHIVE_QUALITY` 规范化为 org 时强制升级（`start(force=True)`，上报 `auto_upgraded`），其余组合（同档/降级/在途/失败）一律跳过；升级失败自动回滚到原 ready 档（不断流，`errors` 留痕）；失败逐项记录 `errors` 并视为已知（**不自动重试**，防永久失败条目重复扣 GP）；手动 `POST /api/favorites/sync/run` 单飞行与周期任务互斥，周期关闭时仍可用。
- **架构**：`app/favorites/state.py`（原子 JSON 快照）+ `app/favorites/sync.py`（周期任务/单飞行/归档作用域门控/去重/容错）+ `app/favorites/router.py`（`/api/favorites/*`）；写操作实现位于 `app/eh/client.py::add_favorite/move_favorite/remove_favorite/fetch_favorite_categories`，聚合在 `EHService.favorite_action/favorite_categories/scan_favorites`；favcat 解析在 `app/eh/parser.py::parse_favcat_map/parse_favorites_categories`。
...
| `GET /opds/v1.2/archives?page=` | **本地归档 shelf（零上游）**：ready zip 母本按 `created_at` 倒序（`page` 分页，对齐 toplist）。条目含 stream/thumb/chapters 链接——源站删本后仍可完整离线阅读；根导航在 store 非空时自动出现、为空时隐藏 |
