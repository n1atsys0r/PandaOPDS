...
- **自动归档**：`FAVORITES_SYNC_ARCHIVE`（默认 0）**必须手动开启**才对新发现条目调用 `ArchiveManager.start()`（GP 消耗风险显式知情）；归档前先过 `FAVORITES_SYNC_CATEGORIES` 作用域（名单外跳过，只建档不归档）；双重去重（快照 `archived` 集合 + archive store 已存在条目任一命中即跳过）；失败逐项记录 `errors` 并视为已知（**不自动重试**，防永久失败条目重复扣 GP）；手动 `POST /api/favorites/sync/run` 单飞行与周期任务互斥，周期关闭时仍可用。
